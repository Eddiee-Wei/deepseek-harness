...
DSH Desktop 是基于 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 的独立社区应用，由社区维护，未获得 DeepSeek 的赞助、背书或审批。仓库首页、App 元数据与帮助菜单、App 资源和 DMG 都会标识上游项目并收录其 MIT License。`ATTRIBUTION.txt` 是随包归属说明，`DEEPSEEK_HARNESS_LICENSE`、`THIRD_PARTY_NOTICES.md` 和 `NODE_LICENSE` 则收录对应的许可证文本。DeepSeek、DeepSeek Harness 及相关标识归各自权利人所有。

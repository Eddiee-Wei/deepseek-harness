...
The desktop release stays close to upstream: most updates are ordinary workspace dependency and Web UI changes, and the native shell remains a small process supervisor and renderer. The signed artifact is self-contained, starts without a user-installed Node.js or runtime download, and closes the most direct loopback-confusion path. The cost is a larger Apple Silicon-only DMG, a macOS-specific build, and Developer ID/notarization secret management. The first release has no automatic updater, Intel binary, App Store sandbox, or promise that an arbitrary remote deployment can use the local token fence as its authentication system.

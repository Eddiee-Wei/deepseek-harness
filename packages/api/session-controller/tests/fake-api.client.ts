  ...
  sessionRemotes(): RuntimeRemotes {
    return {
      ...
      workspace: {
        ...
        repository: payload => this.record(
          'workspace.repository',
          payload,
          Promise.resolve(ok({ repository: { kind: 'directory' } })),
        ),
        openPathWith: payload => this.record(
          'workspace.openPathWith',
          payload,
          Promise.resolve(ok({ opened: true })),
        ),
        createBranch: payload => this.record(
          'workspace.createBranch',
          payload,
          Promise.resolve(ok({
            repository: {
              kind: 'git', root: '/f/ws', branch: payload.branch, detached: false, dirty: false,
            },
          })),
        ),
        createWorktree: payload => this.record(
          'workspace.createWorktree',
          payload,
          Promise.resolve(ok({ workspace: fakeWorkspace(payload.branch) })),
        ),

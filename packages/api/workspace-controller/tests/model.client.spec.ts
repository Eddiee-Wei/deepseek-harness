...
class FakeWorkspaceRemote implements WorkspaceRemote {
  ...
  readonly repository: WorkspaceRemote['repository'] = request => Promise.resolve(remoteOk({
    repository: { kind: 'git', root: `/w/${String(request.workspaceId)}`, branch: 'main', detached: false, dirty: false },
  }))

  readonly openPathWith: WorkspaceRemote['openPathWith'] = () => Promise.resolve(remoteOk({ opened: true }))

  readonly createBranch: WorkspaceRemote['createBranch'] = request => Promise.resolve(remoteOk({
    repository: { kind: 'git', root: `/w/${String(request.workspaceId)}`, branch: request.branch, detached: false, dirty: false },
  }))

  readonly createWorktree: WorkspaceRemote['createWorktree'] = request => Promise.resolve(remoteOk({

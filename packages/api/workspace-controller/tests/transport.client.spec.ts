...
class ScriptedWorkspaceRemote implements WorkspaceRemote {
  ...
  readonly repository: WorkspaceRemote['repository'] = () => { throw new Error('unused') }
  readonly openPathWith: WorkspaceRemote['openPathWith'] = () => { throw new Error('unused') }
  readonly createBranch: WorkspaceRemote['createBranch'] = () => { throw new Error('unused') }
  readonly createWorktree: WorkspaceRemote['createWorktree'] = () => { throw new Error('unused') }
...
class CommandWorkspaceRemote implements WorkspaceRemote {
  ...
  readonly repository = vi.fn<WorkspaceRemote['repository']>(() => Promise.resolve(remoteOk({
    repository: { kind: 'directory' },
  })))

  readonly openPathWith = vi.fn<WorkspaceRemote['openPathWith']>(() => Promise.resolve(remoteOk({ opened: true })))

  readonly createBranch = vi.fn<WorkspaceRemote['createBranch']>(request => Promise.resolve(remoteOk({
    repository: { kind: 'git', root: '/work', branch: request.branch, detached: false, dirty: false },
  })))

  readonly createWorktree = vi.fn<WorkspaceRemote['createWorktree']>(request => Promise.resolve(remoteOk({
    workspace: workspace(request.branch),
  })))

...
import WorkspaceController, { type WorkspaceControllerInternals } from '../src/index.ts'
...
async function harness(internals: WorkspaceControllerInternals = {}) {
  ...
  const controller = new WorkspaceController(ctx, internals)
...
describe('WorkspaceController commands', () => {
  it('resolves registered paths for repository inspection and native opening', async () => {
    const run = vi.fn<NativeCommandRunner>(async () => {
      throw Object.assign(new Error('not a repository'), { code: 128, stderr: '' })
    })
    const openPathWith = vi.fn<NonNullable<WorkspaceControllerInternals['openPathWith']>>()
      .mockResolvedValue(undefined)
    const { controller, root } = await harness({ run, openPathWith })
    const path = stageDir(root, 'plain-directory')
    const created = await controller.create({ path })
    const signal = new AbortController().signal

    await expect(controller.repository({ workspaceId: created.workspace.workspaceId }, signal))
      .resolves.toEqual({ repository: { kind: 'directory' } })
    await expect(controller.openPathWithApplication({
      workspaceId: created.workspace.workspaceId,
      application: 'finder',
    }, signal)).resolves.toEqual({ opened: true })
    expect(openPathWith).toHaveBeenCalledWith(path, 'finder', signal, expect.objectContaining({ run }))
  })

  it('maps Git mutation refusals to Workspace Remote errors', async () => {
    const run = vi.fn<NativeCommandRunner>(async (_command, args) => {
      ...
      if (args.includes('status')) return { stdout: ' M tracked.ts\n', stderr: '' }
      return { stdout: '', stderr: '' }
    })
    const { controller, root } = await harness({ run })
    const created = await controller.create({ path: stageDir(root, 'dirty-repository') })

    await expect(controller.createBranch({
      workspaceId: created.workspace.workspaceId,
      branch: 'feature/ui',
    }, new AbortController().signal)).rejects.toMatchObject({
      code: 'workspace/git-dirty',

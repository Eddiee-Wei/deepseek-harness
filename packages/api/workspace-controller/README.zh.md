...
仓库操作会在 Host 上通过已注册的 Workspace id 解析路径，并用固定可执行文件名和参数数组调用 `git` 与 macOS `open`。创建分支和关联 worktree 要求 Git worktree 干净，且分支名有效、未被占用；关联 worktree 创建在 Harness 持有的家目录下，并且只在 Git 成功后注册。

...
The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection plugin, plugin bundles and the HMR event stream are routes owned by the modules/hmr plugins, and dist serving belongs to the fallback owner. The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). When `accessToken` is configured, every HTTP and upgrade request requires the host-only `dsh_access` cookie. A top-level GET whose `token` query matches in constant time exchanges that bootstrap secret for an HttpOnly, SameSite=Strict cookie and redirects to the same URL without the token; the unconfigured browser server retains its existing behavior. This server serves browsers only; Electron loads dist over `file://` and carries fetch over an IPC bridge. This package never prints; the URL line belongs to the shell.
...
- **No TLS or general origin policy** — the optional token fence protects an owned local browser session, but it is not a remote-deployment authentication system. Binding a non-loopback address exposes the server to that network; deployment hardening (or fronting it with a real reverse proxy) remains out of scope for the dev-facing v1.

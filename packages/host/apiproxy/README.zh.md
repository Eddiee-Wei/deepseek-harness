...
`host.openPathWith` 是面向已注册 Workspace 路径的 macOS 开发者交接，只接受封闭的应用 id（`vscode`、`cursor`、`finder` 或 `terminal`）。Host 会把该 id 映射为固定应用名，并在不经过 shell 的情况下调用 `open -a`。`workspace.repository` 使用参数向量形式的 Git 命令检查已注册路径；`workspace.createBranch` 会先拒绝非仓库、脏 worktree、无效分支名或已有分支，再执行 `git switch -c`；`workspace.createWorktree` 则先在 Harness 持有的家目录下创建新分支及关联 worktree，再把所得路径注册为另一个 Workspace。上述方法都不接收命令字符串或由调用方选择的 worktree 目标目录，并与其他原生 Host 操作一同限定在回环地址。

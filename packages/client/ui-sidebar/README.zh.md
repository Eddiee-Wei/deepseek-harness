...
展开的品牌行把 `sidebar.brand.mark` 与 `sidebar.brand.name` 渲染为两个独立的 single slot，收起轨道则渲染同一个 mark slot。没有占位者时，外壳使用鱼形标记。构建可以通过 `DSH_CLIENT_BRAND_NAME` 与 `DSH_CLIENT_VERSION` 提供可见标签和紧凑的单行版本徽标；版本身份使用独立的紧凑排版，以便在标准侧边栏内完整显示，同时不会折行或压缩徽标。否则，通用的 `DSH Local Build` 标签会携带构建期 7 位 `DSH_CLIENT_COMMIT_HASH` 徽标。部署包可以单独替换任一 slot，而无须替换 New Session 控件或轨道几何；声明感知的 `slots.inject()` 让这种包无论先于还是后于侧边栏激活都能生效。

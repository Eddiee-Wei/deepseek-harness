...
export function apply(ctx: Context): void {
  ...
  async function remoteValue<T>(
    operation: Promise<{ ok: true; value: T } | { ok: false; error: { message: string } }>,
  ): Promise<T> {
  ...
  ctx.slots.inject('conversation.session.header.utilities', () => ctx.slots.register(
    {
      ...
      order: 10,
      locale: NS,
      inject: (): WorkspaceDeveloperControlsInjected => ({
        openPathWith: async (workspaceId, application) => {
          await remoteValue(ctx.remote.workspace.openPathWith({ workspaceId, application }))
        },
        repository: async (workspaceId) => {
          const value = await remoteValue(ctx.remote.workspace.repository({ workspaceId }))
          return value.repository
        },
        createBranch: async (workspaceId, branch) => {
          const value = await remoteValue(ctx.remote.workspace.createBranch({ workspaceId, branch }))
          return value.repository
        },
        createWorktree: async (workspaceId, branch) => {
          const value = await remoteValue(ctx.remote.workspace.createWorktree({ workspaceId, branch }))

...
所选 Session 的区头还会携带 `WorkspaceDeveloperControls`：一枚紧凑的分体启动器，作用于持有该 Session 的已注册 Workspace。其菜单通过本地 Host 在 VS Code、Cursor、Finder 或 Terminal 中打开该目录；设置对话框则报告仓库根目录、当前分支或 detached 状态，以及 clean／dirty 状态。干净仓库可以新建分支并把当前 worktree 切换过去；任何仓库都可以在 Harness 管理的关联 worktree 中创建新分支、把结果注册为 Workspace，并在其中启动 Session。客户端只提供经过校验的分支名和 Workspace id；应用选择、Git 执行、目标目录归属与安全拒绝都由 Host 负责。

...
`openNativePath(path, signal)` hands a path to the default application and prefers the named default browser for HTML and SVG where the platform can identify one. `openNativeTextFile(path, signal)` selects text-editor intent; on macOS it uses `open -t`. `openNativePathInApplication(path, application, signal)` opens a path on macOS in one allowlisted application: Visual Studio Code, Cursor, Finder, or Terminal. WSL paths are translated with `wslpath -w` before the Windows desktop receives them. `canOpenNativePath()` reports whether the current Host plausibly has a desktop target.
...
- [Workspace Controller](../../api/workspace-controller/README.md) — resolves registered Workspace paths before using the allowlisted macOS opener.

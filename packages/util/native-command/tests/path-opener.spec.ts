...
describe('native path opener', () => {
  ...
  it('opens a path in one allowlisted macOS application', async () => {
    const run = vi.fn<PathOpenerRunner>(async () => ({ stdout: '', stderr: '' }))
    await openNativePathInApplication('/Users/test/project', 'vscode', signal(), {
      platform: 'darwin', run,
    })
  ...
  it('rejects named application handoff outside macOS', async () => {
    await expect(openNativePathInApplication('/tmp/project', 'finder', signal(), {
      platform: 'linux',
    })).rejects.toThrow('named application opener is unsupported on linux')
  })
